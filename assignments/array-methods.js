...
runners.forEach(current => {
  fullName.push(`${current.first_name} ${current.last_name}`);
  fullName.sort();
});
...
const allCaps = runners.map(current => {
  return current.first_name.toUpperCase();
});
...
const largeShirts = runners.filter(current => {
  return current.shirt_size == "L";
});
...
// Problem 1
// Get a array of objects with each runner's email and first name

const emails = runners.map(current => {
  return {
...
// Problem 2
// Add another key value pair to each item with a key titled Participating and the value set to true.

runners.forEach(c => {
  return (c.participating = true);
});

console.log(runners);

// Problem 3
// Return an array of only the runners with an ID of 30 or higher

const newRunners = runners.filter(c => {
  return c.id >= 30;
});
